    ...
    for i in range(len(lines)):
        ...
        if(lines[i] != ""):
            if (
                    ...
                    lines[i] == "Ocorreu um erro. Tente recarregar a página." or
...
def horarioParaDate(horario):
    hours = horario[0:2]
    minutes = horario[3:5]
    split_string = horario.split(' ')
    dia = split_string[2]
    month_mapping = {
        'jan': 1,
        'fev': 2,
    ...
    dt = datetime(int(ano), int(mes), int(dia), int(hours), int(minutes))
    return dt
